...
const urlTemplate =
	'https://api.themoviedb.org/3/discover/movie?api_key=<< Your API Key Here >>&language=en-US&sort_by=popularity.desc&include_adult=false&release_date.gte=2020-01-01&release_date.lte=&vote_average.gte=&with_genres=&page=1'
...
function makeReqUrl(params) {
	...
	let genresParamsIds = ''
	for (g of genresList) {
		if (params[g.name.toLowerCase()]) genresParamsIds += `${g.id.toString()},`
	}
	genresParamsIds = genresParamsIds.slice(0, -1)
	let dateFrom = ''
	let dateTo = ''
	if (params.yearFrom) dateFrom = `${params.yearFrom}-01-01`
	if (params.yearTo) dateTo = `${params.yearTo + 1}-01-01`
	...
	let pageMultiplier
	if (params.popular) {
		if (params.pageCount > 50) pageMultiplier = Math.floor(params.pageCount * 0.3)
		else pageMultiplier = params.pageCount + 1
	} else {
